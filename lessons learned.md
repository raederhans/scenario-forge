...
### 2026-04-24 - WSL venv tests on Windows
- 这个仓库的 .venv 是 WSL 创建的 POSIX venv；PowerShell 里的 python -m pytest 可能缺 pytest，直接跑 .venv/bin/python 会卡在 Windows shell。
- 需要运行该 venv 的 pytest 时，用 wsl.exe -e bash -lc 'cd /mnt/c/.../mapcreator && .venv/bin/python -m pytest ...'。
...
### 2026-04-25 - localization extractor drift
- i18n audit 和 translate sync 只要各自维护 declarative attribute 名单，就很容易出现一边已覆盖、一边没收进 `data/locales.json` 的漂移；新增 `data-i18n-*` 能力时，要同步检查 `tools/i18n_audit.py`、`tools/translate_manager.py` 和对应 unittest。
...
- 需要作为 required check 的 workflow 应保持 always-run，由内部 classifier 决定重活或 fast success；pull_request.paths 会让 check 在无关 PR 中缺席。
...
### 2026-04-29 - benchmark page.evaluate return contract
- Playwright `page.evaluate(() => { ... })` 使用块体时必须显式 `return`；性能采样 helper 嵌进去后要用合同测试锁住关键 return，否则会静默写出空 memory/metric 字段。
