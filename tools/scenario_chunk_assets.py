        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if not selected_feature_ids:
                    continue
                chunk_payload = _slice_layer_payload(layer_key, payload, selected_feature_ids)
                ...
                relative_url = f"data/scenarios/{scenario_id}/chunks/{chunk_filename}"
                chunk_entry = {
                    "id": chunk_id,
                    "layer": layer_key,
                    "lod": spec["lod"],
                    ...
                    "bounds": bounds,
                    "priority": 1 if spec["lod"] == "coarse" else 2,
                    "feature_count": len(selected_feature_ids),
                    "data_format": "city_overrides" if layer_key == "cities" else "geojson",
