...
class MainStartupDataPipelineBoundaryContractTest(unittest.TestCase):
    ...
    def test_owner_keeps_startup_data_pipeline_helpers(self):
        ...
        self.assertIn("async function ensureBaseCityDataReady(options = {}) {", owner_content)
        self.assertIn("async function ensureFullLocalizationDataReady(options = {}) {", owner_content)
    ...
    def test_main_keeps_bootstrap_facade_and_owner_wrappers(self):
        ...
        self.assertIn('import { updateUIText } from "./ui/i18n.js";', content)
        self.assertIn("return getStartupDataPipelineOwner().ensureBaseCityDataReady(options);", content)
        self.assertIn("const result = await getStartupDataPipelineOwner().ensureFullLocalizationDataReady(options);", content)
        self.assertIn("updateUIText();", content)
        self.assertIn("return result;", content)
        self.assertIn("return getStartupDataPipelineOwner().ensureActiveScenarioBundleHydrated(options);", content)
        self.assertIn("return getStartupDataPipelineOwner().shouldFastTrackScenarioHydration();", content)
        self.assertIn("return getStartupDataPipelineOwner().ensureContextLayerDataReady(requestedLayerNames, options)", content)
