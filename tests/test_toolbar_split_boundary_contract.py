...
class ToolbarSplitBoundaryContractTest(unittest.TestCase):
    ...
    def test_special_zone_persistence_contract_stays_stable(self):
        ...
        self.assertIn("draft.specialZones = data.specialZones || {}", interaction_funnel)
        self.assertIn("draft.specialZoneLayers = normalizeSpecialZoneLayersState", interaction_funnel)
        self.assertIn("draft.specialZoneMembershipBrushMode = normalizeSpecialZoneMembershipBrushModeState", interaction_funnel)
        self.assertIn("draft.specialRegionOverrides = {};", interaction_funnel)
        self.assertIn('draft.manualSpecialZones = { type: "FeatureCollection", features: [] };', interaction_funnel)
        self.assertIn("restoreImportedStyleConfigState(draft, data.styleConfig);", interaction_funnel)
        self.assertIn("return captureProjectImportState(draft);", interaction_funnel)
        self.assertIn("commitImportedProjectPatch(state, patch);", interaction_funnel)
        import_commit = (REPO_ROOT / "js/core/interaction_funnel/import_apply_orchestration.js").read_text(encoding="utf-8")
        self.assertIn("restoreSpecialZoneImportFields(target, patch);", import_commit)
        self.assertNotIn("state.specialZones = data.specialZones", interaction_funnel)
    ...
    def test_special_zone_membership_tools_have_explicit_renderer_modes(self):
        ...
        self.assertIn("const owner = createBrushInteractionSessionOwner(runtimeState, {", renderer_content)
        self.assertIn("} = composeBrushInteractionSessionOwner();", renderer_content)
        self.assertIn("export function createBrushInteractionSessionOwner(runtimeState, {", brush_content)
    ...
    def test_export_workbench_persistence_contract_stays_stable(self):
        ...
        self.assertIn("restoreImportedWorkbenchUiState(draft, data, {", interaction_funnel)
        self.assertIn("return captureProjectImportState(draft);", interaction_funnel)
        self.assertIn("commitImportedProjectPatch(state, patch);", interaction_funnel)
        import_commit = (REPO_ROOT / "js/core/interaction_funnel/import_apply_orchestration.js").read_text(encoding="utf-8")
        self.assertIn("restoreExportWorkbenchImportFields(target, patch);", import_commit)
        self.assertNotIn("restoreImportedWorkbenchUiState(state, data, {", interaction_funnel)
    ...
    def test_appearance_physical_owner_moves_physical_controls_out_of_controller(self):
        ...
        self.assertIn('void runtimeState.ensureContextLayerDataFn(getPhysicalContextLayerRequests(runtimeState.styleConfig?.physical), { reason: "toolbar-toggle", renderNow: true });', physical_owner_content)
