...
class MapRendererUrbanCityPolicyBoundaryContractTest(unittest.TestCase):
    def test_map_renderer_keeps_facade_while_urban_city_policy_owns_policy_logic(self):
        ...
        self.assertIn("return getUrbanCityPolicyOwner().buildCityRevealPlan(cityCollection, scale, transform, config);", renderer_content)
        self.assertIn("return getUrbanCityPolicyOwner().getEffectiveCityCollection();", renderer_content)
        self.assertIn("return getUrbanCityPolicyOwner().getCityScenarioTag(feature);", renderer_content)
        self.assertIn("return getUrbanCityPolicyOwner().doesScenarioCountryHideCityPoints(tag);", renderer_content)
        self.assertIn("return getUrbanCityPolicyOwner().getUrbanFeatureIndex();", renderer_content)
        self.assertIn("return getUrbanCityPolicyOwner().getCityUrbanRuntimeInfo(feature, urbanIndex);", renderer_content)
