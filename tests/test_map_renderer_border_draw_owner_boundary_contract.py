...
class MapRendererBorderDrawOwnerBoundaryContractTest(unittest.TestCase):
    def test_map_renderer_keeps_border_pass_owner_while_draw_helpers_move_to_owner(self):
        ...
        self.assertIn("return getBorderDrawOwner().drawMeshCollection(meshCollection, strokeStyle, lineWidth, options);", renderer_content)
        self.assertIn("return getBorderDrawOwner().declutterProjectedPolyline(line, minDistancePx, angleThresholdDeg);", renderer_content)
        self.assertIn("return getBorderDrawOwner().getProjectedPolylineMetrics(line);", renderer_content)
        self.assertIn("return getBorderDrawOwner().buildRenderableBoundaryMesh(mesh, {", renderer_content)
        self.assertIn("return getBorderDrawOwner().getViewportAwareCoastlineCollection(collection, k);", renderer_content)
        self.assertIn("return getBorderDrawOwner().getBoundaryMeshTransform(kind, k);", renderer_content)
