...
class MapRendererSpatialIndexRuntimeOwnerBoundaryContractTest(unittest.TestCase):
    def test_map_renderer_keeps_interaction_startup_orchestration_while_spatial_runtime_moves_to_owner(self):
        ...
        self.assertIn("return getSpatialIndexRuntimeOwner().resetSecondarySpatialIndexState();", renderer_content)
        self.assertIn("return getSpatialIndexRuntimeOwner().buildSecondarySpatialIndexes({", renderer_content)
        self.assertIn("return getSpatialIndexRuntimeOwner().buildSpatialIndex({", renderer_content)
        self.assertIn("return getSpatialIndexRuntimeOwner().buildIndexChunked({", renderer_content)
        self.assertIn("return getSpatialIndexRuntimeOwner().buildSpatialIndexChunked({", renderer_content)
