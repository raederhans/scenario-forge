...
class MainBootOverlaySplitBoundaryContractTest(unittest.TestCase):
    ...
    def test_overlay_controller_keeps_dom_and_readonly_contracts(self):
        ...
        self.assertIn("setBootPreviewVisibleState(state, active);", owner_content)
        self.assertIn("setStartupReadonlyStateFields(state, {", owner_content)
        self.assertIn("replaceBootMetricsState(state, {", owner_content)
        self.assertIn("setBootStateFields(state, {", owner_content)
        self.assertIsNone(re.search(r"state\.bootPhase\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootMessage\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootProgress\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootBlocking\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootError\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootCanContinueWithoutScenario\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootMetrics\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.startupReadonly\s*=(?!=)", owner_content))
        self.assertIsNone(re.search(r"state\.bootPreviewVisible\s*=(?!=)", owner_content))
