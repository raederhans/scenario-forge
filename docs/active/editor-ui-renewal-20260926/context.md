...
在推送 `1336e43d` 前后，PR #170 已合并到 `origin/main@c422b9f8`。本分支合入该主线提交以消除 PR 冲突；三份启动词典保留完整合并后的 UI 字典，断言其他场景内容一致后重新生成快照、manifest 和 audit 指纹。业务代码自动合并，图例缓存现在使用主线已完整交付的 helper 契约，34 项支持/语言测试再次通过。主线新增功能属于已合并上游，未从父工作区重新提取未归属改动。
