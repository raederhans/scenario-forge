...
Focused tests run against a Git-blob-verified source snapshot; large canonical data and dist are not present in that local workspace. Browser localhost is denied by the execution environment; native module-worker pixel acceptance must run in an ordinary runner. Neither fixture tests nor ordinary canonical-data CI validate the user's untracked high-precision candidates. No latency, memory or FPS speedup is claimed before controlled target-data measurements.
