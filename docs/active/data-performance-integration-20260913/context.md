...
## Remote integration receipt, 2026-09-13

PR: https://github.com/raederhans/scenario-forge/pull/136, initial head4442f44b. Push succeeded. Administrator merge was rejected because enforce_admins applies to all six required checks; branch protection was not changed. Remote browser smoke, scenario matrix and transport checks passed on the initial head. PR-fast revealed platform gzip header leakage into chunk/startup digests and a historical heavy-test count29 assertion. Main fixed portable gzip OS-byte255 and compares canonical heavy patterns rather than freezing their count; metadata56 cases passed. Follow-up build/tests and PR results are authoritative for the revised head. Local acceptance snapshots above are historical evidence, not current CI status.

Python behavior reference: https://docs.python.org/3/library/gzip.html#gzip.compress (3.11 delegation may emit platform OS byte; 3.13 standardizes255). No hash/size gate was relaxed.
